...
function fixPostalCode(postalCode) {
  let correctedPostalcode = postalCode.trim().toUpperCase();
  let invalidFirsts = `DFIOQWUZ`;
  let invalidChars = `DFIOQU`;
  let firstHalf;
  let secondHalf;
...
function normalizeCoord(value) {
  ...
  if (index !== -1) {
    ...
    secondHalf = value.slice(index + 1, value.length);
  } else {
    index = value.search(`,`);
    secondHalf = value.slice(1, index);
    firstHalf = value.slice(index + 1, value.length - 1);
  }

  if (+firstHalf < -90 || +firstHalf > 90) {
    throw 'Invalid Latitude = ' + firstHalf;
  }
  if (+secondHalf < -180 || +secondHalf > 180) {
    throw 'Invalid Longitude = ' + secondHalf;
  }
...
function formatCoords(...values) {
  ...
  for (let i = 0; i < values.length; i++) {
    try {
      formattedString = normalizeCoord(values[i]);
      returnString += ` `;
